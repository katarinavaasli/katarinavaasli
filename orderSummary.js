...
function printOrderDetails(order) {
    ...
    order.rows.forEach(item => {
        const itemSum = item.price * item.amount * (1 + item.vat);
        totalSum += itemSum;
        
        console.log(`Nimi: ${item.name}, Kogus: ${item.amount}, Hind: ${item.price.toFixed(2)} EUR, Kokku: ${itemSum.toFixed(2)} EUR`);
    });

    console.log(`\nKogusumma: ${totalSum.toFixed(2)} EUR`);
}
